...
func (r *Resend) SendWithContact(ctx context.Context, in ports.OTPSendInput) error {
	...
	if err := r.send(ctx, in.Email, in.PlainOTP, displayName, in.MagicLinkURL); err != nil {
		return err
	}
...
func (r *Resend) send(ctx context.Context, email, plainOTP, displayName, magicLinkURL string) error {
	if r.apiKey == "" {
	...
	body := resendRequest{
		...
		To:      []string{email},
		Subject: subject,
		HTML:    buildOTPHTML(displayName, plainOTP, magicLinkURL),
		ReplyTo: r.replyTo,
	}
